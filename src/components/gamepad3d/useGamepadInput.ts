...
interface ActiveAxis   { zoneId: string; baseCx: number; baseCy: number; hitRadius: number }
interface ActiveButton { zoneId: string; pressedAt: number; isBoolean: boolean }

export function useGamepadInput(
  zonesRef: React.RefObject<Zone[]>,
  controllerId: string,
  publish: (frame: ControllerFrame) => void,
  onButtonDown?: (zone: Zone) => void,
  onInput?: () => void,
) {
  ...
  function publishThrottled(patches: InputPatch[], force: boolean = false) {
    ...
    if (shouldPublish && pendingPatches.current.length > 0) {
      publish(pendingPatches.current);
  ...
  function handleDown(pointerId: number, cx: number, cy: number) {
    ...
    if (zone.inputType === 'joystick') {
      activeAxes.current.set(pointerId, { zoneId: zone.id, baseCx: zone.cx, baseCy: zone.cy, hitRadius: zone.hitRadius });
      axisValues.current.set(zone.id, { x: 0, y: 0 });
      btnPressed.current.set(zone.id, true);
      patches.push({ controllerId, id: zone.id, value: { type: 'axis2d', x: 0, y: 0 } });
      publishThrottled(patches); // Joysticks can be throttled
    } else {
      if (!btnPressed.current.get(zone.id)) {
        onButtonDown?.(zone);
        activeButtons.current.set(pointerId, { zoneId: zone.id, pressedAt: Date.now(), isBoolean: zone.isBoolean });
        btnPressed.current.set(zone.id, true);
        const value: ButtonValue | BooleanValue = zone.isBoolean
          ? { type: 'boolean', pressed: true }
          : { type: 'button',  pressed: true, duration: 0 };
        patches.push({ controllerId, id: zone.id, value });
  ...
  function handleMove(pointerId: number, cx: number, cy: number) {
    ...
    const nx = dist > 6 ? Math.round((dx / dist) * (clamp / maxTravel) * 100) / 100 : 0;
    // Inverting Y to match World Space (Up is Positive) vs Screen Space (Down is Positive)
    const ny = dist > 6 ? Math.round(-(dy / dist) * (clamp / maxTravel) * 100) / 100 : 0;
    const prev = axisValues.current.get(a.zoneId);
    if (prev?.x === nx && prev?.y === ny) return;
    axisValues.current.set(a.zoneId, { x: nx, y: ny });
    publishThrottled([{ controllerId, id: a.zoneId, value: { type: 'axis2d', x: nx, y: ny } as Axis2DValue }]);
    onInput?.();
  }
  ...
  function handleUp(pointerId: number) {
    ...
    if (a) {
      ...
      axisValues.current.delete(a.zoneId);
      btnPressed.current.delete(a.zoneId);
      patches.push({ controllerId, id: a.zoneId, value: { type: 'axis2d', x: 0, y: 0 } as Axis2DValue });
    }
    const b = activeButtons.current.get(pointerId);
    if (b) {
      activeButtons.current.delete(pointerId);
      btnPressed.current.delete(b.zoneId);
      ...
      patches.push({ controllerId, id: b.zoneId, value });
